...
ド・モルガンの法則 : (P ではない または Q ではない) ならば (P かつ Q) ではない
ド・モルガンの法則 (前者 Pではない) (Pである と Qである) = Pである と Pではない が矛盾するから
ド・モルガンの法則 (後者 Qではない) (Pである と Qである) = Qである と Qではない が矛盾するから

mutual
  data _は偶数 : 自然数 → 命題 where
    0は偶数 : 0 は偶数
    一つ前の_ : n は奇数 ならば (1+ n) は偶数

  data _は奇数 : 自然数 → 命題 where
    1は奇数 : 1 は奇数
    一つ前の_ : n は偶数 ならば (1+ n) は奇数
...
言明2 = 4 において 一つ前の 一つ前の 一つ前の 1は奇数 であるから
